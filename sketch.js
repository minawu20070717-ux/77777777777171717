...
let score = 0;
let itemX, itemY;
let itemSpeed = 2.5; // 【修正】初始速度從 4 降到 2.5，大幅放慢掉落速度！
let itemType = ""; 
let itemName = "";
...
function processHandTracking() {
    if (hands.length > 0) {
        ...
        // 【重大修正點】計算手勢距離時，必須用原始未處理的座標計算 dist()，才不會因為畫布鏡像反轉導致數據錯亂！
...
function resetItem() {
    ...
    // 【修正】降低速度遞增幅度，確保即便拿了很多分，掉落速度依然很舒服適中
    itemSpeed = (height * 0.006) + (score * 0.02); 
...
function manageFallingObjects() {
    ...
    let withinX = (itemX > bucketX - bucketWidth / 2 - 15) && (itemX < bucketX + bucketWidth / 2 + 15);
